...
import 'mobile_render_widget.dart';
...
typedef SelectItemCallback = Function(TimelineEntryM mitem);
...
class TimelineWidgetM extends StatefulWidget {
  final MenuItemDataM focusItem;
  final TimelineM timelineM;
  const TimelineWidgetM(this.focusItem, this.timelineM, {super.key});

  @override
  TimelineWidgetMState createState() => TimelineWidgetMState();
}

class TimelineWidgetMState extends State<TimelineWidgetM> {
  ...
  TapTargetM? _touchedBubble;

  /// Syntactic-sugar-getter.
  TimelineM get timelineM => widget.timelineM;
  ...
  void _scaleStart(ScaleStartDetails details) {
    ...
    _scaleStartYearStart = timelineM.start;
    _scaleStartYearEnd = timelineM.end;
    timelineM.isInteracting = true;
    timelineM.setViewport(velocity: 0.0, animate: true);
  }
  ...
  onTouchBubble(TapTargetM bubble) {
    _touchedBubble = bubble;
  }

  onTouchEntryM(TimelineEntryM entryM) {
  }
  ...
  void _tapUp(TapUpDetails details) {
    ...
    if (_touchedBubble != null) {
      if (_touchedBubble!.zoom) {
        MenuItemDataM target = MenuItemDataM.fromEntry(_touchedBubble!.entry);

        timelineM.padding = EdgeInsets.only(
            top: topOverlap +
                devicePadding.top +
                target.padTop +
                TimelineM.parallax,
  ...
  initState() {
    super.initState();
    widget.timelineM.isActive = true;
  }

  /// Update the current view and change the Timeline header, color and background color,
  @override
  void didUpdateWidget(covariant TimelineWidgetM oldWidget) {
                      ...
                      child: Row(
                          ...
                          children: <Widget>[
                            IconButton(
                              ...
                              onPressed: () {
                                widget.timelineM.isActive = false;
