...
typedef TouchBubbleCallback = Function(TapTargetM bubbleM);
typedef TouchEntryCallback = Function(TimelineEntryM entryM);
...
class TimelineRenderObject extends RenderBox {
  ...
  final TicksM _ticksM = TicksM();
  TimelineM? _timelineM;
  MenuItemDataM? _focusItem;
  MenuItemDataM? _processedFocusItem;
  final List<TapTargetM> _tapTargets = [];
  ...
  TimelineM get timeline => _timelineM!;
  MenuItemDataM get focusItem => _focusItem!;
  ...
  set timeline(TimelineM value) {
    if (_timelineM == value) {
      return;
    }
    _timelineM = value;
    updateFocusItem();
    _timelineM?.onNeedPaint = markNeedsPaint;
    markNeedsPaint();
    markNeedsLayout();
  }

  set focusItem(MenuItemDataM value) {
    if (_focusItem == value) {
      return;
    }
    _focusItem = value;
  ...
  void updateFocusItem() {
    ...
    if (_focusItem!.pad) {
      timeline.padding = EdgeInsets.only(
          top: topOverlap + _focusItem!.padTop + TimelineM.parallax,
  ...
  bool hitTestSelf(Offset screenOffset) {
    //touchEntry(null);
    for (TapTargetM bubble in _tapTargets.reversed) {
  ...
  void performLayout() {
    if (_timelineM != null) {
      _timelineM!.setViewport(height: size.height, animate: true);
    }
  }
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    _tapTargets.clear();
    double renderStart = _timelineM!.renderStart;
    double renderEnd = _timelineM!.renderEnd;
    ...
    canvas.save();
    canvas.clipRect(Rect.fromLTWH(offset.dx + _timelineM!.gutterWidth,
        offset.dy, size.width - _timelineM!.gutterWidth, size.height));
    drawItems(
        context,
        offset,
        _timelineM!.entries.cast<TimelineEntryM>(),
        _timelineM!.gutterWidth +
            TimelineM.lineSpacing -
            TimelineM.depthOffset * _timelineM!.renderOffsetDepth,
        scale,
        0);
    canvas.restore();
  }
  ...
  void drawItems(PaintingContext context, Offset offset,
      List<TimelineEntryM> entriesM, double x, double scale, int depth) {
    final Canvas canvas = context.canvas;

    for (TimelineEntryM mitem in entriesM) {
      if (!mitem.isVisible ||
          mitem.y > size.height + TimelineM.bubblesHeight ||
          mitem.endY < -TimelineM.bubblesHeight) {
        /// Don't paint this mitem.
        continue;
      }

      Offset entryOffset = Offset(x + TimelineM.lineWidth / 2.0, mitem.y);

      /// Draw the small circle on the left side of the Timeline.
      canvas.drawCircle(
          entryOffset,
          TimelineM.edgeRadius,
          Paint()
            ..color = (mitem.accent)
                .withOpacity(mitem.opacity));
      ...
      double bubbleHeight = _timelineM!.bubbleHeight(mitem);
      ...
      builder.addText(mitem.name);
      ui.Paragraph labelParagraph = builder.build();
      labelParagraph.layout(const ui.ParagraphConstraints(width: maxLabelWidth));

      double textWidth =
          labelParagraph.maxIntrinsicWidth * mitem.opacity * mitem.labelOpacity;
      double bubbleX = _timelineM!.renderLabelX -
          TimelineM.depthOffset * _timelineM!.renderOffsetDepth;
      double bubbleY = mitem.labelY - bubbleHeight / 2.0;
      ...
      canvas.drawPath(
          ...
          Paint()
            ..color = (mitem.accent)
                .withOpacity(mitem.opacity * mitem.labelOpacity));
      canvas
          .clipRect(Rect.fromLTWH(bubblePadding, 0.0, textWidth, bubbleHeight));
      _tapTargets.add(TapTargetM()
        ..entry = mitem
        ..rect = Rect.fromLTWH(
      ...
      drawItems(context, offset, mitem.children, x + TimelineM.depthOffset,
          scale, depth + 1);
    }
  }
